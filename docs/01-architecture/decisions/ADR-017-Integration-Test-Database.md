...
**Status:** Proposed **Date:** 2026-07-16 **Owner:** Founding Engineer / Principal Architect
**Related Documents:
...
does not name the _mechanism_ that provides the real Postgres. That mechanism is required to verify the persistence
...
- Spring Boot's Docker Compose support MAY additionally back local `bootRun` for developer convenience, but the **test
  gate** is Testcontainers, so tests never depend on a developer's hand-started services.
