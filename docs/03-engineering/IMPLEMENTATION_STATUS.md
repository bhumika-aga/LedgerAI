...
| Phase 1 — Authentication & Profile  | In Progress | ~50%     | Done: authentication slice — register/login/refresh/logout/me, JWT access + rotated refresh (httpOnly cookie, ADR-018), BCrypt, RFC 7807 errors, security config; frontend sign-in + session bootstrap; authorization foundation (ownership guard + principal abstraction) ready for the first protected feature. Remaining: User Profile, protected-route guard, rate limiting (FR-AUTH-008, SHOULD). |
